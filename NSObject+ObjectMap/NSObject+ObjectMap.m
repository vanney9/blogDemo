...
+(id)objectOfClass:(NSString *)object fromJSON:(NSDictionary *)dict {
    ...
    for (NSString *key in [dict allKeys]) {
        ...
        // If it's an array, check for each object in array -> make into object/id
            ...
            NSLog(@"vanney code log... [NSObject+ObjectMap] : what happened : %@", [NSString stringWithFormat:@"propertyArrayMap.%@", key]);
            NSLog(@"vanney code log... [NSObject+ObjectMap] : propertyType : %@", propertyType);
...
-(NSString *)typeFromProperty:(objc_property_t)property {
    NSLog(@"%@", [NSString stringWithUTF8String:property_getAttributes(property)]);
